    ...
    public class JobsController : Controller
    {
        ...
        public ActionResult Index(int? id)
        {
            ...
            if (Session["FromDetails"] != null && Session["DynModel"] != null)
            {
                ...
                _JobList = objJobMod.GetJobDetails(objJob).OrderByDescending(x => x.JobId).ToList();
                if (_JobList.Count == 1)
                {
        ...
        public JsonResult PostedJobs(string _searchtext, string trendingtagsid)
        {
            ...
            objJobList = MatchBxCommon.GenerateBadge(objJobMod.GetJobDetails(objJob).OrderByDescending(x => x.JobId).ToList());
            _JobList = objJobList;
        ...
        public JsonResult LoadMoreJobs(int id,string _searchtext, string trendingtagsid)
        {
            ...
            objJob.SkillsList = "0";
            List<Job> _JobList = new List<Job>();
            objJobList = MatchBxCommon.GenerateBadge(objJobMod.GetJobDetails(objJob).OrderByDescending(x=>x.JobId).Where(x => x.JobId < id).ToList());
        ...
        public ActionResult GetTagsnSkills(int category, int jobId)
        {
            ...
            if (jobId != 0)
            {
                TempData["Tags"] = _TrendingTagsModel.GetList(" * ", " JobCategoryId = " + category);
                _objJobList.TrendingTagsList = _TrendingTagsModel.GetList(" * ", " JobCategoryId = " + category);
            } else
            {
                TempData["Tags"] = _TrendingTagsModel.GetList(" * ", " JobCategoryId = " + category + " and TagType = 'S'");
            ...
            TempData["Skills"] = _SkillsModel.GetList(" * ", " JobCategoryId = " + category);
