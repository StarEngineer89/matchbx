...
--EXEC spGetCompletedGigSubscription 58

IF EXISTS (SELECT * FROM sys.procedures where schema_id = schema_id('dbo')and name=N'spGetCompletedGigSubscription')
DROP procedure spGetCompletedGigSubscription
GO
CREATE PROCEDURE spGetCompletedGigSubscription
(	 
	@GigId int
)
AS
BEGIN
	...
	WHERE GS.GigId = @GigId AND J.JobStatus = 'C'
