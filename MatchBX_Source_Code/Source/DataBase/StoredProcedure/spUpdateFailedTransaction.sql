...
BEGIN
	...
	DECLARE @UserId INT,@JobTitle NVARCHAR(200),@Notification NVARCHAR(300),@Url NVARCHAR(50),@GigSubscriptionId INT
	...
	BEGIN
		...
		UPDATE GigSubscription SET GigSubscriptionStatus = 'A' WHERE GigSubscriptionId = @GigSubscriptionId		
		
		--Notification send				
		SELECT @UserId =UserId,@JobTitle = JobTitle FROM Job WHERE JobId = @JobId
		
		SET @Url = CAST('/Jobs/Details/' + CAST(@JobId AS NVARCHAR(50)) AS NVARCHAR(50))
		SET @Notification = 'Your transaction for ' + @JobTitle + ' has failed and your subscription has been relisted.'
		INSERT INTO MatchBXNotification VALUES (@UserId,@UserId,@Notification,0,'Transaction Failed',GETDATE(),@Url)
		
		--Delete the job corresponding to subscription
