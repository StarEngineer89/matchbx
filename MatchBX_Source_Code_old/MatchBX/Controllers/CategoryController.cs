   ...
   // [CustomExceptionFilter]
    public class CategoryController : Controller
    {
        ...
        public ActionResult Index(int? id)
        {
            ...
            try
            {
                objJobList = MatchBxCommon.GenerateBadge(objJobMod.GetJobDetails(objJob).ToList().OrderByDescending(x => x.BudgetASP).ThenByDescending(x => x.JobId).ToList());
            }
            catch (Exception e)
            {
            ...
            objTrending.FromPage = category == "G" ? "G" : "J";
            ...
            objSkills.FromPage = category == "G" ? "G" : "J";
        ...
        public JsonResult ApplyFilter(string topskills, decimal budgetMin, decimal budgetMax, string populartags, string sortorder, string _searchtext)
        {
            ...
            objJob.FromPage = category == "G" ? "G" : "J";
            objJobList = MatchBxCommon.GenerateBadge(objJobMod.GetJobDetails(objJob));
            objJobList.ForEach(s => s.BudgetASPInt = Convert.ToInt32(s.BudgetASP));
            if (sortorder == "H")
            {
                objJobList = objJobList.OrderByDescending(x => x.BudgetASP).ThenByDescending(x=>x.JobId).ToList();
            }
            else
            {
            ...
            if (string.IsNullOrWhiteSpace(_searchtext))
            {
                ...
                if (sortorder == "H")
                {
                    objJobList = objJobList.OrderByDescending(x => x.BudgetASP).ThenByDescending(x => x.JobId).Take(_RecordDisplay).ToList();
                }
                else
                {
                    if (objJob.FromPage == "J")
                    {
                        objJobList = objJobList.OrderByDescending(x => x.JobId).Take(_RecordDisplay).ToList();
                    }
                    else
                    {
        ...
        public JsonResult LoadMoreJobs(int id, string topskills, decimal budgetMin, decimal budgetMax, string populartags, string sortorder, string _searchtext)
        {
            ...
            if (sortorder == "H")
            {
                ...
                index1 = objJobList.FindIndex(x => x.JobId == id);
            ...
            {
                objJobList = objJobList.Where(x => x.JobId < id).ToList();
               _TotalRecord = objJobList.Where(x => x.JobDescription.ToLower().Contains(_searchtext.ToLower().Trim()) || x.JobTitle.ToLower().Contains(_searchtext.ToLower().Trim()) || x.JobCompletionDateDisplay.ToLower().Contains(_searchtext.ToLower().Trim()) || x.FullName.ToLower().Contains(_searchtext.ToLower().Trim())).ToList().Count();
            ...
            else
            {
                ...
                else
                {
                    if (category == "J")
                    {
                        _TotalRecord = objJobList.Where(x => x.JobId < id).ToList().Count();
                        if (_TotalRecord > _RecordDisplay)
                        {
                            _loadmore = 1;
                        }
                        ...
                        model.Job = objJobList.OrderBy(x => x.BudgetASPInt).Where(x => x.JobId > id).Take(_RecordDisplay);
                    }
                }
